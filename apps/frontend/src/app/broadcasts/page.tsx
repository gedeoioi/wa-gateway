...
export default function BroadcastsPage() {
  ...
  useEffect(() => {
    ...
    socket.on('broadcast:completed', (d: { broadcastId: string }) => {
    ...
    <DashboardLayout title="Broadcasts" subtitle="Send messages to multiple recipients">
              ...
              <div>
                ...
                {form.deviceId && devices.find((d) => d.id === form.deviceId)?.status !== 'connected' && (
                  <p className="text-xs text-amber-600 mt-1">
                    This device is not connected. Connect it first before starting the broadcast.
                  </p>
                )}
              </div>
              <div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={
            <button onClick={() => setShowCreate(true)} className="bg-whatsapp hover:bg-whatsapp-dark text-white px-4 py-2 rounded-lg">
              New Broadcast
            </button>
          }
        />
                ...
                <div className="flex gap-2">
                  ...
                  {bc.status === 'sending' && (
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 text-xs text-yellow-600 bg-yellow-50 rounded-lg hover:bg-yellow-100"
                    >
